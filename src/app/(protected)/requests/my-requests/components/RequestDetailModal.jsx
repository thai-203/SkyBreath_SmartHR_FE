...
export default function RequestDetailModal({ isOpen, request: initialRequest, onClose, onRefresh, canApprove = false, onEdit }) {
  ...
  const handleRevoke = async () => {
    setSubmitting(true);
    try {
      await requestsService.revoke(
        request.id,
        showRevokeForm.levelOrder,
        comment,
      );
  ...
  if (
    ...
    levels = previewLevels.map((p) => ({
      ...
      status: "PENDING",
      approver: p.approverEmployee
        ? {
          fullName: p.approverEmployee.fullName,
          avatar: p.approverEmployee.avatar,
        }
        : null,
    }));
  }
  ...
  const myCurrentLevel = levels.find(
    (l) =>
      l.approverEmployeeId === currentEmployee?.id &&
      l.levelOrder === request?.currentApprovalLevel,
  );
  const isMyTurn =
    !!myCurrentLevel &&
    myCurrentLevel.status === "PENDING" &&
            ...
            loading ? (
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <InfoRow
                    label="Tính công"
                    value={request?.isWorkedTime ? "Có" : "Không"}
                  />
                  <InfoRow
                    label="Từ (Bắt đầu)"
                    value={`${request?.startDate || "—"} ${request?.startTime || ""}`}
                  />
                  <InfoRow
                    label="Đến (Kết thúc)"
                    value={`${request?.endDate || "—"} ${request?.endTime || ""}`}
                  />
                  <InfoRow
            ...
            <div className="shrink-0 border-t px-6 py-4 bg-slate-50 space-y-3">
              ...
              {canApprove && request?.status === "PENDING" && (
                showRejectForm ? (
                  ...
                  <PermissionGate permission="REQUEST_APPROVE">
