...
export default function TimesheetTable({
    ...
    onLock,
    onDelete,
    mode = "all", // "all", "generation", "data", "locking"
}) {
    const columns = useMemo(
            ...
            cols.push({
                ...
                cell: ({ row }) => (
                    <div className="flex items-center gap-1">
                        {(mode === "all" || mode === "data" || mode === "locking") && (
                        ...
                        {!row.original.isLocked && (
                            <>
                                {(mode === "all" || mode === "data") && authService.hasPermission("TIMESHEET_UPDATE") && (
                                    <Button variant="ghost" size="icon" onClick={() => onRecalculate(row.original)} title="Tính lại">
                                        <RefreshCw className="h-4 w-4 text-amber-500" />
                                    </Button>
                                )}
                                {(mode === "all" || mode === "generation") && authService.hasPermission("TIMESHEET_CREATE") && (
                        ...
                        {(mode === "all" || mode === "locking") && authService.hasPermission("TIMESHEET_LOCK") && (
                            <>
                                {row.original.isLocked ? (
        ...
        [onViewDetail, onRecalculate, onLock, onDelete, pagination, mode]
    );
        ...
        <Card>
            <CardHeader className="border-b border-slate-200">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    {!authService.hasAnyRole(['ADMIN', 'HR']) && authService.hasAnyRole(['EMPLOYEE']) ? null : (
