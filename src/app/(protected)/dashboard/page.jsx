...
export default function DashboardPage() {
  ...
  const shiftStart = attendanceContext?.shift?.startTime;
  const shiftEnd = attendanceContext?.shift?.endTime;
              ...
              <CardContent className="space-y-4 pt-6">
                {topDepartments.length === 0 ? (
                  ...
                  topDepartments.map((department) => {
                    const maxCount = topDepartments[0]?.totalEmployeeCount || 1;
                    const width = Math.max(
                      8,
                      Math.round(
                        ((department.totalEmployeeCount || 0) / maxCount) * 100,
                      ),
                    );
