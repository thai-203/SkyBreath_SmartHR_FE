...
export default function ShiftGroupsPage() {
  ...
  const handleEdit = (item) => {
    setSelected(item);
    setFormData({ groupName: item.groupName, description: item.description || "" });
    setIsFormOpen(true);
  };
    ...
    <div className="p-6 space-y-6 bg-slate-50/50 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-blue-600 mb-1">
            <LayoutGrid size={18} />
            <span className="text-[11px] font-bold uppercase tracking-wider">Thiết lập tổ chức</span>
          </div>
          <PageTitle title="Quản lý nhóm ca" className="text-2xl font-bold text-slate-800" />
          <p className="text-slate-500 text-sm mt-1">Tổ chức và phân loại các nhóm làm việc theo ca kíp.</p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="px-5 py-3 bg-slate-50 border-t border-slate-100 flex items-center gap-2">
          ...
          <span className="text-[12px] text-slate-500 italic">Nhấn vào biểu tượng bút chì để chỉnh sửa hoặc thùng rác để xóa nhóm ca.</span>
        </div>
      </div>
