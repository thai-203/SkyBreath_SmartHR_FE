...
export default function OnboardingProgressTable({
  progressList = [],
  loading,
  onRowClick,
}) {
  ...
  const currentItems = progressList.slice(
    startIndex,
    ...
    return (
      <div className="p-20 text-center border-2 border-dashed border-slate-100 rounded-3xl m-8">
        ...
        <p className="text-slate-400 font-medium">
          Không có tiến trình onboarding nào đang chạy
        </p>
      </div>
    );
  }
        ...
        <table className="w-full text-left border-separate border-spacing-y-3">
          <thead>
            <tr className="text-slate-400">
              <th className="px-6 py-2 text-[10px] font-bold uppercase tracking-widest">
                Nhân sự
              </th>
              <th className="px-6 py-2 text-[10px] font-bold uppercase tracking-widest">
                Phòng ban & Vị trí
              ...
              <th className="px-6 py-2 text-[10px] font-bold uppercase tracking-widest w-1/5">
                Tiến độ
              </th>
              <th className="px-6 py-2 text-[10px] font-bold uppercase tracking-widest text-center">
                Chi tiết
              </th>
            </tr>
          </thead>
          ...
          <tbody>
            {currentItems.map((p) => {
              const displayMeta = getProgressDisplayMeta(p);
              return (
                <tr
                  key={p.id}
                ...
                >
                  ...
                  <td className="px-6 py-4 border-y border-transparent group-hover:border-indigo-100">
                    <div className="flex flex-col">
                      ...
                      {displayMeta.isOverdue && (
                        <div className="mt-2 inline-flex items-center gap-1 text-[11px] font-bold text-rose-600 bg-rose-50 border border-rose-100 rounded-md px-2 py-1 w-fit">
                          Quá hạn {displayMeta.overdueDays} ngày
                        </div>
                      )}
                    </div>
                  </td>
                  ...
                  {/* Trạng thái */}
                  <td className="px-6 py-4 border-y border-transparent group-hover:border-indigo-100">
                    <StatusBadge status={displayMeta.status} />
                  </td>
