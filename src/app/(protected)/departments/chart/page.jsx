...
function OrgNode({ department, level = 0, viewType = "rut-gon" }) {
  ...
  if (!viewType || viewType === "rut-gon") {
    return (
      <div className="relative">
        <div
          className={`flex items-center gap-3 p-3 rounded-lg border border-[var(--border)] bg-white hover:shadow-md transition-all duration-200 ${
            level === 0 ? "shadow-lg" : ""
          }`}
        >
          {hasChildren && (
  ...
  const borderLeftColorClass =
    LEVEL_COLORS_LEFT[level % LEVEL_COLORS_LEFT.length];

  if (viewType === "ngang") {
    return (
      <li>
        <div
          className={`node-content mx-auto w-max bg-white rounded-lg shadow-sm border border-slate-200 border-t-4 ${borderColorClass} relative z-10 transition-all hover:shadow-md min-w-[200px]`}
        >
          <div className="p-3 text-center border-b border-slate-100">
            <h4 className="font-semibold text-slate-800 text-sm whitespace-nowrap">
              {department.departmentName}
            </h4>
          </div>
          <div className="flex justify-between items-center text-[11px] text-slate-600 font-medium bg-slate-50/50 rounded-b-lg">
            <span
              className="px-1.5 py-1.5 border-r border-slate-200 flex-1 text-center"
              title="Tổng nhân sự"
            >
              <span className="text-blue-500">TNS:</span> {tns}
            </span>
            <span className="px-1.5 py-1.5 flex-1 text-center" title="Thử việc">
              <span className="text-indigo-500">TT:</span> {tt}
            </span>
          </div>
          {hasChildren && (
            <div
  ...
  return (
    <div className="flex items-start mb-4 relative org-tree-doc-node">
      <div
        className={`node-content bg-white w-[250px] shrink-0 rounded-lg shadow-sm border border-slate-200 border-l-4 ${borderLeftColorClass} relative z-10 transition-all hover:shadow-md`}
      >
        <div className="p-3 border-b border-slate-100 flex items-center justify-between">
          <h4
            className="font-semibold text-slate-800 text-sm whitespace-nowrap truncate"
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent
          ...
          className="p-6 overflow-x-auto bg-slate-50 min-h-[600px] border border-slate-200 shadow-inner rounded-xl"
        >
          {loading ? (
