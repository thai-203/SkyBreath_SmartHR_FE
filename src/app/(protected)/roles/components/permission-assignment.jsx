...
export default function PermissionAssignmentDialog({
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const rolePermissionIds = (rolePermissionsMsg.data || []).map(
        (p) => p.id,
      );
    ...
    setSelectedPermissions((prev) => {
      ...
      if (!isChecking && action === "VIEW") {
        ...
        const viewPerm = permissions.find(
          (p) =>
            (p.module || "General") === module &&
            getAction(p.permissionCode) === "VIEW",
        );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>Giao quyền cho vai trò này.</DialogDescription>
        </DialogHeader>
              ...
              <div className="space-y-6 p-1">
                {Object.entries(groupedPermissions).map(([module, perms]) => (
                  <div key={module} className="space-y-3">
                    <h4 className="font-semibold text-sm text-foreground uppercase tracking-wider bg-muted/50 p-2 rounded">
                      {module}
                    </h4>
