...
export default function PenaltiesPage() {
  ...
  const fetchPenalties = useCallback(
    async (searchValue, filtersValue, page) => {
      try {
        setLoading(true);
        const params = { page, limit: PAGE_SIZE };

        if (searchValue?.trim()) params.search = searchValue.trim();
        if (filtersValue.violationType)
          params.violationType = filtersValue.violationType;
  ...
  const violationLabel =
    deleteModal.data?.violationType === "LATE" ? "Đi muộn" : "Về sớm";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <PermissionGate permission="PENALTY_CREATE">
