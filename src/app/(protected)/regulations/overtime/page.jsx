...
export default function OvertimeRulesPage() {
  ...
  const fetchRules = useCallback(async (searchValue, filtersValue, page) => {
    try {
      ...
      if (filtersValue.versionStatus)
        params.versionStatus = filtersValue.versionStatus;
      if (filtersValue.overtimeTypeId)
        params.overtimeTypeId = filtersValue.overtimeTypeId;
      if (filtersValue.departmentId)
        params.departmentId = filtersValue.departmentId;
      if (filtersValue.minMultiplier)
        params.minMultiplier = filtersValue.minMultiplier;
      if (filtersValue.maxMultiplier)
        params.maxMultiplier = filtersValue.maxMultiplier;
      if (filtersValue.minHoursPerDay)
        params.minHoursPerDay = filtersValue.minHoursPerDay;
      if (filtersValue.maxHoursPerDay)
        params.maxHoursPerDay = filtersValue.maxHoursPerDay;
      if (filtersValue.minHoursPerMonth)
        params.minHoursPerMonth = filtersValue.minHoursPerMonth;
      if (filtersValue.maxHoursPerMonth)
        params.maxHoursPerMonth = filtersValue.maxHoursPerMonth;
  ...
  const handleSubmitForm = async () => {
    ...
    const ruleData = {
      ...
      overtimeTypeId: formData.overtimeTypeId
        ? parseInt(formData.overtimeTypeId)
        : undefined,
  ...
  const handleConfirmActivate = async () => {
    ...
    try {
      ...
      showError(
        err.response?.data?.message || "Không thể kích hoạt quy định này",
      );
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <PermissionGate permission="OVERTIME_RULE_CREATE">
