...
export default function OvertimeTable({
  ...
  const hasActiveFilters =
    ...
    filters.maxMultiplier ||
    filters.minHoursPerDay ||
    filters.maxHoursPerDay ||
    filters.minHoursPerMonth ||
    filters.maxHoursPerMonth;
  ...
  const getDynamicStatus = (rule) => {
    ...
    return {
      label: rule.versionStatus || "Unknown",
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) =>
                  handleFilterChange("overtimeTypeId", e.target.value)
                }
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) =>
                  handleFilterChange("versionStatus", e.target.value)
                }
                className="h-9 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
              >
                <option value="">Tất cả</option>
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  onChange={(e) =>
                    handleFilterChange("minMultiplier", e.target.value)
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleFilterChange("maxMultiplier", e.target.value)
                  }
                  className="h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
                />
              </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              ...
              <PermissionGate
                permission={["OVERTIME_RULE_UPDATE", "OVERTIME_RULE_DELETE"]}
              >
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-500">
                  Thao tác
                </th>
              ...
              data.map((rule, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-center">
                    {rule.overtimeType ? (
                      <span
                        className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${overtimeTypeColors[rule.overtimeType.code] || "bg-slate-100 text-slate-600"}`}
                      >
                    ...
                    {rule.effectiveFrom ? (
                      <div className="text-xs">
                        ...
                        <div className="text-slate-400">
                          {rule.effectiveTo
                            ? `Đến: ${formatDate(rule.effectiveTo)}`
                            : "Vô thời hạn"}
                        </div>
                      </div>
                    ) : (
                  ...
                  <td className="px-4 py-3 text-center">
                    {(() => {
                      ...
                      return (
                        <span
                          className={`inline-flex items-center rounded-md px-2.5 py-1 text-xs font-semibold tracking-wide ${status.color}`}
                        >
                          {status.label}
                        </span>
                      );
                  ...
                  >
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {rule.versionStatus === "DRAFT" && onActivate && (
                          <PermissionGate permission="OVERTIME_RULE_UPDATE">
                            <Button
                              variant="ghost"
                              size="icon"
                        ...
                        {rule.versionStatus === "DRAFT" && (
                          <PermissionGate permission="OVERTIME_RULE_DELETE">
                            <Button
                              variant="ghost"
                              size="icon"
